...
This project generates a video as shown in the above GIF of 9:16 aspect ratio and automatically publishes it to Instagram using Github as a data storage. It also allows you to create an image quote as well. In `src/input.py` you can edit all parameters of the video / image, they are:
...
Other options for video generation are available to be edited as well but a little more technical. View them inside `src/input.py`. The audio is chosen by gemini api from the `audios/` folder. The `images/` folder contains subdirectories of images categorized by different genres. You can choose the subfolder yourself or use the `random` variable to randomly select from all subfolders with regex `images[0-9]*/*` inside `src/input.py`.

Video publishment:

- `caption`: Contains quote, a text asking for a follow, and a description about the author. It is given from a function inside the `util.py` file. The description about the author is generated by AI.
...
You will need a gemini api key inside your `.env` so that a background audio can be chosen automatically. But if you don't want that, edit the `audio` variable inside `src/input.py` and manually point to a .mp3 file inside `audios/`.

Once the environment is setup, configure the parameters in `src/input.py` and run the `src/vid_generator.py` file to generate the video.
